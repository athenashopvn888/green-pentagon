...
export function getItemData(category: string, name: string): ItemEffects {
  ...
  return {
    effects: menuNotes,
    description: `${name} appears in the ${categoryLabel} category at Green Pentagon Cannabis. Review the current menu and package details before visiting.`,
    metaDescription: `Review ${name} menu information at Green Pentagon Cannabis on Queen Street West. Check current details before visiting.`,
